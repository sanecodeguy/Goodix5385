...
bool fingerprint_match(string img_path, string clear_path, string sample_path) {
    ...
    for (const auto& file : recursive_directory_iterator(sample_path)) {
        if (((string)file.path()).substr(((string)file.path()).size()-4, 4) == ext) {
            ...
            if (keypoints_match.size() < MIN_MATCH) continue;
            ...
            if (matches.size() < MIN_MATCH) continue;
            ...
            for (int j = 0; j < matches.size(); j++) {
                ...
                for (int k = j+1; k < matches.size(); k++) {
                    ...
                    if (1 - min(length_1, length_2) / max(length_1, length_2) <= LENGTH_MATCH) {
            ...
            if (angles.size() < MIN_MATCH) continue;
            ...
            for (int j = 0; j < angles.size(); j++) {
                ...
                for (int k = j+1; k < angles.size(); k++) {
                    ...
                    if (1 - min(angle_1.sin, angle_2.sin) / max(angle_1.sin, angle_2.sin) <= ANGLE_MATCH &&
                        1 - min(angle_1.cos, angle_2.cos) / max(angle_1.cos, angle_2.cos) <= ANGLE_MATCH) {
                            count += 1;
                        if (count >= MIN_MATCH)
                            return true;
                    }
...
int main(int argc, char** argv) {
    if (argc < 4) {
        cout << "Usage: " << argv[0] << " <fingerprint_image> <clear_image> <templates_dir>" << endl;
        return 1;
    }
