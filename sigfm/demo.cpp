...
void update(int, void *) {
    ...
    auto distance_match = (double)cv::getTrackbarPos("distance match", "match") / 100;
    ...
    for (auto match_in : matches_in)
        if (match_in[0].distance < distance_match * match_in[1].distance) {
            ...
            if (std::find(matches_out.begin(), matches_out.end(), match_out) == matches_out.end())
                matches_out.push_back(match_out);
        }

    auto length_match = (double)cv::getTrackbarPos("length match", "match") / 100;
    auto angle_match = (double)cv::getTrackbarPos("angle match", "match") / 100;
    auto max_count = 0;
    std::vector<std::pair<cv::Point2f, cv::Point2f>> max_matches;
    for (auto match_1 : matches_out) {
        std::vector<std::pair<double, std::pair<cv::Point2f, cv::Point2f>>> angles;
        for (auto match_2 : matches_out) {
            if (match_1 == match_2) continue;
            ...
            if (length_1 > length_2) std::swap(length_1, length_2);
        ...
        for (auto angle_1 : angles) {
            auto count = 1;
            std::vector<std::pair<cv::Point2f, cv::Point2f>> matches = {match_1};
            ...
            if (count > max_count) { max_count = count; max_matches = matches; }
        }
    }
...
int main(int argc, char** argv) {
    std::string fp_path = argc > 1 ? argv[1] : "fingerprints/";
    path = fp_path;
    clear = cv::imread(path + "clear" + ext, cv::IMREAD_GRAYSCALE);

    if (clear.empty()) {
        printf("No clear image found at %sclear.pgm\n", path.c_str());
        return -1;
    }
