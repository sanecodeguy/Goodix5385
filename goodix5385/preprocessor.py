...
def preprocess_image(bg_path: str, img_path: str, output_prefix: str = "processed"):
    bg_width, bg_height, bg_depth, bg_img = tool.read_pgm(bg_path)
    width, height, depth, raw_img = tool.read_pgm(img_path)
    ...
    total = width * height
    print(f"Coverage: {coverage} out of {total}, {100 * coverage // total}%")
    ...
    print(f"White level: {white_lvl}")

    res = hist_equalization(subtracted, black_lvl, white_lvl)

    mean_1 = mean_filter(res, width, height, 1, [1, 2, 1, 2, 4, 2, 1, 2, 1])
    result = subtract_image(mean_1, res, 0, 100, 200)

    output_path = f"{output_prefix}.pgm"
    tool.write_pgm(result, height, width, output_path)
    return output_path
