...
class Device:
    ...
    def _recv_message_from_device(self, timeout: float | None):
    ...
    def _send_message_to_device(self, message: Message, use_checksum: bool, ack_timeout: float):
    ...
    def _send_mcu(self, data_type, data: bytes):
        ...
        payload = struct.pack("<L", data_type)
        payload += struct.pack("<L", len(data) + 8)
        payload += data
    ...
    def execute_fdt_operation(self, fdt_op: FingerDetectionOperation, fdt_base: bytes, timeout: float = 0):
        ...
        payload += int.to_bytes(1, length=1, byteorder="little")
        payload += fdt_base

        self._send_message_to_device(Message(0x3, fdt_op.value, payload), True, ack_timeout)
    ...
    def wait_for_fdt_event(self, fdt_op: FingerDetectionOperation, timeout: float | None = None):
        return self._get_finger_detection_data(fdt_op, timeout)

    def _get_finger_detection_data(self, fdt_op: FingerDetectionOperation, timeout: float | None):
        reply = self._recv_message_from_device(timeout)
    ...
    def erase_app_firmware_info(self, reset_delay: int):
        self._send_message_to_device(
            Message(0xA, 2, b"\x00" + reset_delay.to_bytes(1, byteorder="little")),
            True,
            500,
        )
    ...
    def read_firmware(self, addr: int, size: int) -> bytes:
        ...
        self._send_message_to_device(Message(0xF, 1, msg), True, 500)
    ...
    def update_firmware(self, firmware: bytes):
        ...
        while sent_bytes < len(firmware):
            ...
            self._send_message_to_device(Message(0xF, 0, msg), True, 500)
        ...
        self._send_message_to_device(Message(0xF, 2, msg), True, 500)
...
def _derive_session_key(psk, random_data: bytes, session_key_length: int):
    ...
    while len(session_key) < session_key_length:
        ...
        session_key += Crypto.Hash.HMAC.HMAC(psk, A + seed, Crypto.Hash.SHA256).digest()
...
def _gea_decrypt(key, encrypted_data):
    ...
    for data_idx in range(0, len(encrypted_data), 2):
        ...
        stream_val = (
            (uVar2 >> 8) & 0xFFFF) + (uVar2 & 0xFF | key & 1) * 0x100
